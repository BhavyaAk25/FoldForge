...
## 2026-07-21 — semantic specification and deterministic topology synthesis

- Replaced the production model-authored topology contract with strict `FabricationDesignSpecV3`. Sol can declare semantic parts, dimension ranges, relationships, motion intent, material and sheet constraints, landmarks, priorities, and tolerances, but cannot declare a root, body graph, attachment edge, fold sign, connector geometry, packing, or transform.
- Added a generic bounded synthesizer that enumerates connected acyclic graphs, grounded roots, equal-length physical attachments, fold and connector choices, then runs the unchanged compiler and complete verifier. Canonical structural fingerprints become nogoods so equivalent invalid realizations are not re-evaluated.
- Kept `FabricationPlanV2` only as a code-owned internal synthesis representation and as an explicitly isolated historical-response replay contract. The live model tool accepts only one topology-free V3 design specification.
- Added exact homepage playing-card-box acceptance from a mocked V3 response through real synthesis, compile, full verification, candidate construction, and source-equivalent SVG/DXF/GLB/JSON finalization. Added strict-schema, bounded-failure, role, outline, randomized-dimension, repeatability, and real route-pipeline regressions.
- The no-cost suite passes 558 tests and the 1,000-run seeded property gate. Coverage passes at 96.39% statements, 90.02% branches, 97.74% functions, and 97.75% lines. No OpenAI, live FoldForge route, Vercel, production, external, or paid call was made during implementation or validation.
